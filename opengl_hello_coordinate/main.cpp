...
int main() {
    ...
    // // View matrix: move camera backward
    glm::mat4 view(1.0f); // Identity matrix
    view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
    // // Projection matrix: perspective projection of entire space
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        sp.use();
        int model_loc = glGetUniformLocation(sp.ID, "model");
        glUniformMatrix4fv(model_loc, 1, GL_FALSE, glm::value_ptr(model));
        int view_loc = glGetUniformLocation(sp.ID, "view");
        glUniformMatrix4fv(view_loc, 1, GL_FALSE, glm::value_ptr(view));
        int projection_loc = glGetUniformLocation(sp.ID, "projection");
