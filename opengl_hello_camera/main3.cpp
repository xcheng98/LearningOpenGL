...
// Original Position, direction vector, and up vector
glm::vec3 cameraPos = glm::vec3(0.0f, 0.0f, 3.0f);
glm::vec3 targetDir = glm::vec3(0.0f, 0.0f, -1.0f); // Should be normalized
glm::vec3 upDir = glm::vec3(0.0f, 1.0f, 0.0f);

// Keep track of the time of rendering one frame
float deltaTime = 0.0f;
float lastFrameTime = 0.0f;

// Keep track of the mouse position in last frame
...
void processInput(GLFWwindow* window){
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        cameraPos += cameraSpeed * glm::normalize(glm::cross(glm::cross(targetDir, upDir), targetDir));
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        cameraPos -= cameraSpeed * glm::normalize(glm::cross(glm::cross(targetDir, upDir), targetDir));
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        cameraPos -= cameraSpeed * glm::normalize(glm::cross(targetDir, upDir));
...
void mouse_callback(GLFWwindow* window, double xPos, double yPos)
{
    ...
    targetDir.x = cos(glm::radians(pitch)) * sin(glm::radians(yaw));
    targetDir.y = sin(glm::radians(pitch));
    targetDir.z = cos(glm::radians(pitch)) * cos(glm::radians(yaw));
}
...
int main() {
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(VAO);
        sp.use();
        // Set up view and projection matrix to move the camera and zoom
        view = glm::lookAt(cameraPos, cameraPos + targetDir, upDir);
